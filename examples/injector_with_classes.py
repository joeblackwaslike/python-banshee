...
class GetUserHandler:
    ...
    async def __call__(self, query: GetUserQuery) -> User | None:
        """
        Get user.
...
class DoGreetingHandler:
    def __init__(self, bus: banshee.Bus) -> None:
        self.bus = bus

    async def __call__(self, command: GreetCommand) -> None:
        """
        Do greeting.
